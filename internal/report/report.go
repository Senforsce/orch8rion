...
// FromWorkDir reads the orch8rion files from a `go build -work` directory and creates a [Report] out of it.
...
func fromWorkFS(ctx context.Context, root string, fsys fs.FS) (Report, error) {
	...
	log.Debug().Str("root", root).Msg("reading orch8rion files from work directory")
	...
	for _, packageBuildDir := range entries {
		_ = fs.WalkDir(fsys, filepath.Join(packageBuildDir.Name(), aspect.Orch8rionDirPathElement),
			func(path string, d os.DirEntry, err error) error {
				...
				log.Debug().Str("path", path).Msg("found orch8rion file")
...
type (
	// ModifiedFile represents a file that has been modified by Orch8rion.
	ModifiedFile struct {
		// original is the fullpath of the original file before it was modified by Orch8rion.
		// It can be already deleted in certain cases, such as when the original file was a CGo file that was not modified.
		original string

		// modified is the fullpath of the modified file that was generated by Orch8rion. It can be missing
		// if a build was made without the `-work` flag.
		modified string
	}

	// Report represents a collection of modified files that were generated by Orch8rion.
	// It implements the [fs.FS] interface, allowing it to be used as a file system rooted in "/".
	// This will open every file normally expect for files that have been modified by Orch8rion, in which case it will
...
func (m ModifiedFile) ImportPath() string {
	...
	_, pkg, found := strings.Cut(dir, aspect.Orch8rionDirPathElement)
	if !found {
		return ""
	}
...
func (r Report) Diff(writer io.Writer) error {
	...
	if _, err := exec.LookPath("diff"); err != nil {
		return fmt.Errorf("diff command not found: %w (cannot run orch8rion diff without the diff binary being in the path)", err)
	}
...
// constructed as "<work-dir>/orch8rion/src/<github.com/my/repo>/<file.go>".
